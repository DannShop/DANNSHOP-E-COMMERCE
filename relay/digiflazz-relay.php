<?php
/**
 * Relay IP tetap untuk panggilan API Digiflazz.
 *
 * MASALAH YANG DISELESAIKAN
 * Digiflazz menolak request dari IP yang tidak terdaftar di whitelist mereka
 * (rc 45: "IP Anda tidak kami kenali"). Aplikasi berjalan di Vercel, yang TIDAK
 * punya IP keluar tetap - tiap invocation bisa keluar dari IP AWS mana saja, jadi
 * whitelist satu IP hari ini pasti gagal lagi besok.
 *
...
// Host yang boleh dituju. Dikunci ke Digiflazz supaya file ini TIDAK bisa dipakai
// sebagai open proxy ke alamat sembarangan kalau URL-nya sampai bocor - tanpa ini,
// siapa pun yang tahu secret-nya bisa memakai hosting kamu untuk menyerang situs
// lain, dan yang tercatat sebagai penyerang adalah IP kamu.
const ALLOWED_HOSTS = ['api.digiflazz.com'];

// Path API yang boleh diteruskan. Daftar tertutup, bukan pola.
const ALLOWED_PATHS = ['/v1/transaction', '/v1/price-list', '/v1/cek-saldo'];
...
if (!is_array($req) || !isset($req['url']) || !is_string($req['url']) || !isset($req['body']) || !is_array($req['body'])) {
    reply(400, ['ok' => false, 'error' => 'Body harus JSON berisi {url, body}.']);
}
