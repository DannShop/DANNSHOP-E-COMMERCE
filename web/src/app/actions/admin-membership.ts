...
const tierFieldsSchema = z.object({
  ...
  // Potongan flat rupiah, khusus produk MANUAL. 0 = tidak dipakai.
  discountFlatManual: z.coerce.bigint().min(0n, "Potongan flat tidak boleh negatif"),
...
export async function createMembershipTier(formData: FormData): Promise<ActionResult> {
  ...
  const parsed = createSchema.safeParse({
    ...
    discountFlatManual: formData.get("discountFlatManual") || 0,
  ...
  const tier = await db.membershipTier.create({
    data: {
      ...
      discountFlatManual: parsed.data.discountFlatManual,
