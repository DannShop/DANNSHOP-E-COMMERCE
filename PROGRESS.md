...
Terakhir update: 2026-07-29 (Fase 1-4 SELESAI TOTAL, sudah di-merge ke `main` LOKAL (commit `55e8a08`) dan di-push ke GitHub. **Fase 7 dipecah jadi sub-fase 7a-7d — spec + plan Fase 7a SUDAH DITULIS & di-commit, EKSEKUSI BELUM DIMULAI (belum ada worktree/branch) — lihat kotak "⚠️ LANGKAH PERTAMA SESI BERIKUTNYA" di bawah.**)

## ⚠️ LANGKAH PERTAMA SESI BERIKUTNYA (jangan lewati/lupa)

Fase 1-4 sudah selesai total dan aman di `main` (lokal + GitHub). Fase 5 (provider 2-4) terhambat dependensi eksternal (dokumentasi/kredensial member area Serpul/OkeConnect/QiosPay belum diambil user). User memilih lanjut ke **Fase 7 dulu**, dipecah jadi 4 sub-fase: **7a** (admin orders + refund queue + notifikasi Telegram — spec+plan SUDAH SELESAI DITULIS), 7b (job alert saldo + notifikasi email), 7c (hardening keamanan), 7d (laporan) — belum ada spec untuk 7b-7d.

**7a: spec di `docs/superpowers/specs/2026-07-29-fase-7a-admin-orders-refund-design.md`, plan 8-task di `docs/superpowers/plans/2026-07-29-fase-7a-admin-orders-refund.md` — KEDUANYA SUDAH DI-COMMIT ke `main`, TAPI EKSEKUSI BELUM DIMULAI SAMA SEKALI** (belum ada worktree, belum ada branch, belum ada task-brief/ledger SDD). User berhenti tepat sebelum menjawab pilihan mode eksekusi (Subagent-Driven vs Inline) untuk istirahat. **Begitu user bilang lanjut Fase 7a: tanya lagi pilihan mode eksekusi itu** (jangan asumsikan Subagent-Driven meski itu yang direkomendasikan), lalu ikuti proses standar (buat worktree via `EnterWorktree` kalau Subagent-Driven, baca plan, jalankan `superpowers:subagent-driven-development` dari Task 1).

Detail penting dari plan 7a (baca plan lengkap sebelum eksekusi, ini cuma ringkasan): 8 task — (1) fungsi pure `decideFulfillmentRetry` TDD, (2) modul `sendTelegramAlert`/`formatOrderAlertMessage` TDD, (3) wire alert Telegram ke 4 titik NEEDS_REVIEW/REFUND_PENDING yang sudah ada di `fulfillment.ts`/`runner.ts`, (4) `retryOrderFulfillment`+`retryOrderRefund` (refactor `dispatchFulfillment` jadi `selectAndSend` reusable), (5) server actions `web/src/app/actions/orders.ts`, (6) halaman `/admin/orders`, (7) halaman `/admin/orders/[orderNumber]` + 4 tombol aksi, (8) env var Telegram + verifikasi E2E manual (butuh setup bot Telegram asli via @BotFather). Temuan penting saat riset plan: `NEEDS_REVIEW` punya 3 penyebab beda di kode (bukan 2 seperti dugaan spec awal) — ditangani via 1 tombol "Coba Lagi" pintar (`decideFulfillmentRetry`) alih-alih tombol ke-5, supaya UX tetap simpel.
